 ...
 * @param {Boolean} options.async Forces the requests to be synchronous or asynchronous (ignores timing if synchronous).
...
export default class Server {
  ...
  config(config = {}) {
    ...
    */
    ...
    /**
      ...
      ```
    */
    this.urlPrefix = config.urlPrefix || this.urlPrefix || "";

    /**
      Set the number of milliseconds for the the Server's response time.
      Or set to `false` to make the request synchronous instead.

      By default there's a 400ms delay during development, and a `false` delay in testing (so your tests run instantly).
      ...
      @public
    */
    this.timing = config.timing !== undefined ? config.timing : 400;

    /**
      Set to `true` or `false` to explicitly specify the requests as synchronous or asynchronous.

      By default (if async is undefined) the request is asynchronous during development, and synchronous in testing.
      An asynchronous request will use the timing property as a delay, while a synchronous request will force the timing to false.
    ...
    */
    if (this.async === undefined) {
      this.async = config.async === undefined ? undefined : !!config.async;
    }
  ...
  */
   ...
   * In the test environment, it sets timing to 0 if requests are explicitly asynchronous, or set it to 0 otherwise.
   * Outside of the test environment, it sets timing to false if requests are explicity synchronous, or keeps its original value otherwise.
   * environment.
   *
   * @method loadConfig
  ...
  loadConfig(config) {
    ...
    if (this.isTest()) {
      this.timing = this.async === true ? 0 : false;
    } else {
      this.timing = this.async === false ? false : this.timing;
    }
  }
